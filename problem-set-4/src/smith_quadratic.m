function [et,v,k1,x] = smith_quadratic(x0,k0,t,k_grid)
%%
% SMITH_LINEAR.M
...
%
tic;
x = fminunc(@(x) myfun_quadratic(x,k0,t),x0,optimset('MaxIter',1000,'MaxFunEvals',1000,'LargeScale','off','HessUpdate','bfgs'));
% constructing the value function
ngrid = length(k_grid);
v = zeros(ngrid,1);
for i = 1:ngrid;
    v(i) = -myfun_quadratic(x,k_grid(i),t);
end;
k1 = x(1) + x(2)*k_grid;
toc;
et = toc;
