...
k_0 = 0.01;
k_T = (alpha*beta/(1-beta*(1-delta)))^(1/(1-alpha));
step = (k_T-k_0)/(ngrid-1);
k_grid = (k_0:step:k_T)';
n_grid = k_grid*(1/(alpha*beta/(1-beta*(1-delta)))^(1/(1-alpha)));
%--------------------------------------------------------------------------
% value function implementation
%--------------------------------------------------------------------------
[et,it,vf,p,n,c] = vi(k_grid,n_grid);
disp('Raw VI done ...');
...
plot(k_grid,polk,'k--');
xlim([k_0 k_T])
l1 = legend('VI','Location','NorthWest');
set(l1,'box','off');
xlabel('capital grid');
