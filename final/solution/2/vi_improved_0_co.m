function [et,it,vf,p] = vi_improved_0_co(k_grid)
%%
% VI_IMPROVED_0_CO.M
...
% USAGE     [et,it,vf,p] = vi_improved_0_co(k_grid)
...
p = zeros(ngrid,1);
    ...
    for i = 1:ngrid;
        vstore = -inf*ones(ngrid,1);
        for j = tag:ngrid;                                                  % exploiting monotonicity of the policy function
            c = k_grid(i).^alpha+(1-delta)*k_grid(i)-k_grid(j);
            if c > 0;
                vstore(j) = retrn(k_grid(i),k_grid(j)) + beta*v0(j) - 0.5*1000*( max([(1-delta)*k_grid(i)-k_grid(j);0]).^2 );
            end;
